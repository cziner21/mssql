...
namespace mssqlteszt
{
    public partial class modify : Form
    {
        ...
        private void fillAllTbx() {
            ...
            /*List<string> varosazonosito = new List<string>();
            foreach (DataGridViewRow row in dataGridView1.Rows) {
                //MessageBox.Show((row.Cells[6].Value).ToString());
                if (row.Cells[6].Value != null && !varosazonosito.Contains(row.Cells[6].Value)) {

                    varosazonosito.Add((row.Cells[6].Value).ToString());
                }
                
            }
            foreach (string varos in varosazonosito) {
                varosIdCbx.Items.Add(varos);
            }*/

            //MessageBox.Show(diakIdCbx.SelectedValue.ToString());
